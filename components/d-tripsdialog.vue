...
const handleForm = async (method) => {
  ...
  if (!isFormValid.value || selected.value.length === 0) {
    await createDialog({
      ...
      buttons: [{ title: "Ok", key: "ok" /* any v-btn api option */ }],
    });
  ...
  if (
    method === "PUT" &&
    props.item.expenses.some(
      (expense) => !selected.value.some((user) => user.id === expense.userId)
    )
  ) {
  ...
  if (method === "PUT") {
    rec.id = props.item.id;
  }

  // console.log('tripsdialog, rec:',rec)
  // Send data to API
  try {
    await $ifetch.request("/api/trips", {
      method: method,
      data: rec,
